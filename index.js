...
function convertLength(value) {
    ...
    const result = `${value} meters = ${feet.toFixed(3)} feet | ${value} feet = ${meter.toFixed(3)} meters`
    length.textContent = result
}
function convertVolume(value) {
    const gallon  = value * 0.264172
    const liter =  value * 3.78541
    const result = `${value} liters = ${gallon.toFixed(3)} gallons | ${value} gallon = ${liter.toFixed(3)} liters`
    volume.textContent = result
}
function convertMass(value) {
    const pound  = value * 2.20462
    const kilo =  value* 0.453592
    const result = `${value} kilos = ${pound.toFixed(3)} pounds | ${value} pounds = ${kilo.toFixed(3)} kilos`
